...
public class RundeSchachtel extends Kreis implements Schachtel {
    ...
    /**
     * VB: hoehe, name und durchmesser sind nicht null und hoehe und durchmesser >0 NB: eine RundeSchachtel
     * mit den uebergebenen Parametern wurde erzeugt
     * 
     * @param hoehe
     ...
     * @param durchmesser
     *            Durchmesser der zu erzeugenden Schachtel
     */
    @DevelopedBy("Christoph Huber")
    ...
    public boolean passtHinein(Geschenk g) {
        if (this.getHoehe() < g.getHoehe() + 1 || (this.getHoehe() >= g.getHoehe() + 3)) {
            ...
            if (this.getDurchmesser() > k.getDurchmesser() + 1
                && this.getDurchmesser() <= k.getDurchmesser() + 3) {
                return true;
            } else {
                return false;
            }
            ...
            if (this.getDurchmesser() > s.getUmkreisDurchmesser() + 1
                && this.getDurchmesser() <= s.getUmkreisDurchmesser() + 3) {
                return true;
            } else {
                return false;
            }
